...
-- PUBLIC FUNCTIONS --
function DnD.Init(wtMovable, wtReacting, fUseCfg, fLockedToParentArea, Padding,
                  KbFlag, Cursor, oldParam1, oldParam2)
    if wtMovable == DnD then
        wtMovable, wtReacting, fUseCfg, fLockedToParentArea, Padding, KbFlag, Cursor, oldParam1 =
            wtReacting, fUseCfg, fLockedToParentArea, Padding, KbFlag, Cursor,
            oldParam1, oldParam2
    end
    ...
    if not DnD.Widgets then
        ...
        common.RegisterEventHandler(DnD.OnResolutionChanged,
    ...
    DnD.Widgets[ID].CfgName = fUseCfg and "DnD:" ..
                                  DnD.GetWidgetTreePath(
                                      DnD.Widgets[ID].wtMovable)
    DnD.Widgets[ID].fLockedToParentArea =
        fLockedToParentArea == nil and true or fLockedToParentArea
    DnD.Widgets[ID].KbFlag = type(KbFlag) == "number" and KbFlag or false
    DnD.Widgets[ID].Cursor = Cursor == false and "default" or type(Cursor) ==
                                 "string" and Cursor or "drag"
    ...
    elseif type(Padding) == "number" then
        for i = 1, 4 do DnD.Widgets[ID].Padding[i] = Padding end
    end
    ...
    if fUseCfg then
        ...
        if Cfg then
            ...
            DnD.Widgets[ID].wtMovable:SetPlacementPlain(
                DnD.NormalizePlacement(InitialPlace, LimitMin, LimitMax))
        end
    end
...
function DnD.IsDragging() return DnD.Dragging and true or false end
-- FREE BONUS --
...
function DnD.GetWidgetID(wtWidget)
    ...
    for ID, W in pairs(DnD.Widgets) do
        if W.wtReacting:GetInstanceId() == WtId or W.wtMovable:GetInstanceId() ==
            WtId then return ID end
    end
end
...
function DnD.GetParentRealSize(Widget)
    ...
    if parent then
        ...
        ParentSize.sizeX = (ParentRect.x2 - ParentRect.x1) *
                               DnD.Screen.fullVirtualSizeX /
                               DnD.Screen.realSizeX
        ParentSize.sizeY = (ParentRect.y2 - ParentRect.y1) *
                               DnD.Screen.fullVirtualSizeY /
                               DnD.Screen.realSizeY
    else
        ParentRect = {
            x1 = 0,
            y1 = 0,
            x2 = DnD.Screen.realSizeX,
            y2 = DnD.Screen.realSizeY
        }
...
function DnD.PrepareLimits(ID, Place)
    local LimitMin = {}
    local LimitMax = {}
    local ParentSize, ParentRect = DnD.GetParentRealSize(DnD.Widgets[ID]
                                                             .wtMovable)
    ...
    elseif Place.alignX == WIDGET_ALIGN_LOW_ABS then
        LimitMin.posX = Padding[4] * DnD.Screen.realSizeX /
                            DnD.Screen.fullVirtualSizeX
        LimitMax.posX = (ParentSize.sizeX - Place.sizeX - Padding[2]) *
                            DnD.Screen.realSizeX / DnD.Screen.fullVirtualSizeX
    end
    ...
    elseif Place.alignY == WIDGET_ALIGN_LOW_ABS then
        LimitMin.posY = Padding[1] * DnD.Screen.realSizeY /
                            DnD.Screen.fullVirtualSizeY
        LimitMax.posY = (ParentSize.sizeY - Place.sizeY - Padding[3]) *
                            DnD.Screen.realSizeY / DnD.Screen.fullVirtualSizeY
    end
...
function DnD.OnPickAttempt(params)
    ...
    if DnD.Widgets[Picking] and DnD.Widgets[Picking].Enabled and
        (not DnD.Widgets[Picking].KbFlag or DnD.Widgets[Picking].KbFlag ==
            KBF_NONE and params.kbFlags == KBF_NONE or (common.GetBitAnd and
            common.GetBitAnd(params.kbFlags, DnD.Widgets[Picking].KbFlag) ~= 0 or
            bit.band(params.kbFlags, DnD.Widgets[Picking].KbFlag) ~= 0)) then
        ...
        common.RegisterEventHandler(DnD.OnDragCancelled,
...
function DnD.OnDragTo(params)
    ...
    if DnD.Widgets[DnD.Dragging].fLockedToParentArea then
        DnD.Place =
            DnD.NormalizePlacement(DnD.Place, DnD.LimitMin, DnD.LimitMax)
    end
    DnD.Widgets[DnD.Dragging].wtMovable:SetPlacementPlain(DnD.Place)
    common.SetCursor(DnD.Widgets[DnD.Dragging].Cursor)
end
function DnD.OnDropAttempt() DnD.StopDragging(true) end
function DnD.OnDragCancelled() DnD.StopDragging(false) end
function DnD.StopDragging(fSuccess)
    ...
    common.UnRegisterEventHandler(DnD.OnDragCancelled,
                                  "EVENT_DND_DRAG_CANCELLED")
    if fSuccess then
